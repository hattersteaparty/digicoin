...
#if defined(NDEBUG)
# error "Digicoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5216d3e33712d8a130680a050ca60fc9ef3aa5a2aa23d9e94c4f13419efdf243");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Digicoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Digicoin Signed Message:\n";
...
{
    // Digicoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Digicoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Digicoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Digicoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Digicoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Digicoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7fc68ff7a83bd3a590e0f53ebaa12e220b70aaedcb85024decd599cc6151cd48");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = " 4/22/22 Punishing Disney, DeSantis Signals a Lasting G.O.P. Brawl With Business";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04ad717d5eb65f50327bc6be50241e1e333f4160d1924f142b993afe4a41d8221c59c282228b166d58b6ac67b1739ece5c43ce0e2a699622ed7eba35430e38f5bc") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1650674824;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084941896;

        if (fTestNet)
        {
            block.nTime    = 1650674779;
            block.nNonce   = 386573544;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x709bef26d71f020829e578305abd418f64de4e53a95fb099cbca916c93e78419"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc7, 0xb0, 0xdl }; // Digicoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DigicoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DigicoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static DigicoinMiner(CWallet *pwallet)
{
    printf("DigicoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("digicoin-miner");
    ...
    try { loop {
        ...
        printf("Running DigicoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DigicoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&DigicoinMiner, pwallet));
}
